...
export default function(store: Store<Model>) {
  ...
  options.on('select', (node: Node) => {
    ...
    const pkg = select.getFocusedPackage(state);
    const noCommand = "echo 'no command available'";
    if (pkg) {
      let com;
      let c;
      switch (node.name) {
        case 'Build':
          c = (pkg.scripts || {}).build || noCommand;
          logger.log(`Running build in ${pkg.cwd}`);
          com  = command.runInPackage(pkg.cwd, c);
          break;
        case 'Lint':
          c = (pkg.scripts || {}).lint || noCommand;
          logger.log(`Running linter in ${pkg.cwd}`);
          com  = command.runInPackage(pkg.cwd, c);
          break;
        case 'Cover':
          c = (pkg.scripts || {}).test || noCommand;
          logger.log(`Running coverage in ${pkg.cwd}`);
          com  = command.runInPackage(pkg.cwd, c);
          break;
        default:
  ...
  function setFocus(state: Model) {
    const focussedPackage = select.getFocusedPackage(state);
    if (focussedPackage) {
      focussed.setData({
        extended: true,
